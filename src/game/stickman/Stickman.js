...
export class Stickman {
  constructor(x, y, id, color, name, isAI = false) {
    ...
    // Speed trail history ΓÇö stores past joint snapshots for ghost smear effect
    this.trailHistory = [];
  ...
  update(groundY, screenWidth, opponent, effectSystem, platforms = []) {
    ...
    // Speed trail sampling ΓÇö capture joint snapshot during fast movement or attacks
    const fastStates = [
  ...
  checkAttackCollision(opponent, effectSystem) {
    ...
    if (hit) {
      ...
      if (isParry) {
        ...
        // ΓöÇΓöÇΓöÇ Normal Block ΓöÇΓöÇΓöÇ
  ...
  computeTargetJoints() {
    ...
    if (this.state === STATES.IDLE) {
      // Karate guard stance ΓÇö bent knees, guard hands raised
      ...
      // Stumble pose ΓÇö off-balance, arms splayed
      const p = this.animProgress;
      ...
      // Phase 1 (0..14): Jab punch ΓÇö right hand forward
      // Phase 2 (15..28): Cross punch ΓÇö left hand forward  
  ...
  draw(ctx) {
    ...
    // ΓöÇΓöÇ Speed Trails (Smear Effect) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    if (this.trailHistory.length > 0) {
      const trailBones = [
    ...
    if (this.state !== STATES.DEAD) {
      ...
      // ΓöÇΓöÇΓöÇ Weapon & Unarmed Swing Trails (smears) ΓöÇΓöÇΓöÇ
      if (isAttacking) {
        ctx.save();
        ...
        if (this.weapon) {
          ...
          // ΓöÇΓöÇΓöÇ Unarmed Fight Smears ΓöÇΓöÇΓöÇ
          if (isPunching) {
      ...
      // ΓöÇΓöÇΓöÇ Draw Equipped Weapons ΓöÇΓöÇΓöÇ
      if (this.weapon) {
        ctx.save();
        ctx.translate(hand.x, hand.y);
